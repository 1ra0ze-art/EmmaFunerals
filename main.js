...
function renderGallery() {
  ...
  products.forEach((p, i) => {
    const thumb = p.imgs && p.imgs[0]
      ? `<img src="${p.imgs[0]}" alt="${p.name}" onerror="this.parentElement.innerHTML='<div class=pc-no-img>⚰</div>'">`
      : '<div class="pc-no-img">⚰</div>';
    grid.innerHTML += `
  ...
  if (!imgs.length) {
    ...
    if (imgs.length > 1) {
      slides.innerHTML += `
        ...
        <div class="slide-arrows">${imgs.map((_,idx) =>
          `<button class="slide-dot ${idx===0?'active':''}" onclick="goSlide(${idx})"></button>`
        ).join('')}</div>`;
    }
  }
...
function cartCheckout() {
  ...
  const wa = cart[0].wa ? cart[0].wa.replace(/\D/g,'') : '250788871345';
